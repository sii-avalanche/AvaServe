...
def is_dcp_lse_merge_phase(forward_batch: ForwardBatch, use_dsa: bool) -> bool:
    ...
    LSE-merged attention path as decode / target-verify -- the DSA backend
    never consumes the dense prefix gather
    (``all_gather_kv_cache_for_mla_extend``; only ``flashinfer_mla_backend``
    does), so the decode machinery is exact there. The DSA phase set is
    single-sourced in ``dsa.utils.should_merge_dsa_dcp_lse``.
    """
    if use_dsa:
        return should_merge_dsa_dcp_lse(forward_batch.forward_mode)
    return is_dcp_mla_decode_phase(forward_batch)
...
class DeepseekMLAForwardMixin:
    ...
    def forward_absorb_prepare(
        ...
        if get_parallel().dcp_enabled:
            # DSA (WQ Hopper DCP): plain EXTEND also takes the Q all-gather +
            # LSE merge path; the DSA backend has no consumer for the dense KV
            # gather.
