...
class CommonKVSender(BaseKVSender):
    def __init__(
        ...
        if get_parallel().dp_size > 1 and not req_has_disagg_prefill_dp_rank:
            worker_dp_rank = self._get_prefill_dp_rank()
            if get_parallel().load_balance_method != "follow_bootstrap_room":
                self._register_prefill_dp_rank()
            elif worker_dp_rank != self.bootstrap_room % get_parallel().dp_size:
                ...
                if envs.SGLANG_DISAGGREGATION_FORCE_QUERY_PREFILL_DP_RANK.get():
                    ...
                    self.kv_mgr.record_failure(
                        ...
                        f"{worker_dp_rank} but bootstrap_room "
    ...
    def _get_prefill_dp_rank(self) -> int:
        if self.kv_mgr.server_args.enable_dp_attention:
            return self.kv_mgr.attn_dp_rank
        return self.kv_mgr.system_dp_rank
    ...
    def _register_prefill_dp_rank(self):
        ...
        payload = {
            ...
            "dp_rank": self._get_prefill_dp_rank(),
        }
        try:
